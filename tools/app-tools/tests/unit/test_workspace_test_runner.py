...
def runner(tmp_path):
    for manifest in [*ROOT.glob("packages/*/*/pyproject.toml"), *ROOT.glob("tools/*/pyproject.toml")]:
        (tmp_path / manifest.parent.relative_to(ROOT) / "tests").mkdir(parents=True)
    ...
    def run(db="sqlite", module="all", paths=(), **settings):
        env = {k: v for k, v in os.environ.items() if k not in {"COVERAGE", "DOCKER", "TEST_JOBS", "PYTEST_OPTIONS"}}
