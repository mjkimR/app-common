...
# app-common workspace root: .../app-common/tools/app-tools/tests/integration/create_code/test_generate.py
...
def test_generated_feature_typechecks_with_pyright(tmp_path):
    ...
    result = subprocess.run(
        ["uv", "run", "--no-active", "--no-sync", "pyright", "--project", str(tmp_path)],
        cwd=str(REPO_ROOT),
        capture_output=True,
        text=True,
    )
