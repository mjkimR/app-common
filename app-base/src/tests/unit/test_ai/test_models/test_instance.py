...
from app_base.ai.models.instance import (
    close_ai_client,
    get_ai_client,
    reload_ai_client,
    set_ai_client,
...
def reset_global_ai_client():
    import app_base.ai.models.instance as ai_instance
...
def test_get_ai_client_lazily_initializes_default_client(mocker, mock_client):
    mock_ai_client_cls = mocker.patch("app_base.ai.models.instance.AIClient", return_value=mock_client)
...
def test_setup_ai_client_already_initialized(mocker, mock_client, tmp_path):
    set_ai_client(mock_client)
    mock_ai_client_cls = mocker.patch("app_base.ai.models.instance.AIClient")
...
def test_close_ai_client(mock_client):
    ...
    with (
        ...
        monkeypatch.setattr("app_base.ai.models.instance.setup_ai_client", lambda: None)
        get_ai_client()
