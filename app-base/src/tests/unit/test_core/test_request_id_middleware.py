...
from app_base.core.middlewares.request_id_middleware import RequestIDMiddleware, add_middleware
...
def test_request_id_logged_on_start_and_end():
    with patch("app_base.core.middlewares.request_id_middleware.logger") as mock_logger:
        client = _make_app()
        client.get("/test")
