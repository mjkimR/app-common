...
from app_base.adapter.nosql_db.repository import NoSQLRepository, CreateSchemaType, ModelType, UpdateSchemaType
...
class BaseNoSQLUpdateServiceMixin(
    ...
    BaseNoSQLServiceMixinInterface,
    Generic[TNoSQLRepo, ModelType, UpdateSchemaType, TContextKwargs],
):
    async def update(
        self,
        provider: NoSQLDBProvider,
        ...
        **update_fields: Any,
        ...
        ctx = self._ensure_context(context, self.context_model)
        async with self._context_update(provider, document_id, obj_data, context=ctx):
            extra_fields = self._prepare_update_fields(obj_data, context=ctx, **update_fields)
            obj = await self.repo.update(provider, document_id=document_id, obj_in=obj_data, **extra_fields)
            return await self._post_update(provider, obj, context=ctx)
