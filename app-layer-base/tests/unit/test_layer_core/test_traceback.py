"""Unit tests for app_layer_base.core.traceback"""

from unittest.mock import MagicMock, patch

from app_layer_base.core.traceback import (
    _is_whitelisted,
...
class TestGetExceptionTracebackStrFullMode:
    ...
    def test_returns_full_traceback(self):
        with patch("app_layer_base.core.traceback.get_app_settings", return_value=_mock_settings(simple=False)):
            try:
...
class TestGetExceptionTracebackStrFilteredMode:
    ...
    def test_starts_with_filtered_header(self):
        with patch("app_layer_base.core.traceback.get_app_settings", return_value=_mock_settings()):
            try:
    ...
    def test_noise_patterns_filtered_out(self):
        ...
        with (
            patch("app_layer_base.core.traceback.get_app_settings", return_value=_mock_settings()),
    ...
    def test_external_lib_filtered_without_whitelist(self):
        ...
        with (
            patch("app_layer_base.core.traceback.get_app_settings", return_value=_mock_settings(whitelist=[])),
    ...
    def test_whitelisted_external_lib_included(self):
        ...
        with (
            patch(
                "app_layer_base.core.traceback.get_app_settings", return_value=_mock_settings(whitelist=["sqlalchemy"])
            ),
