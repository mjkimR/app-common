...
from app_vector_store.lifespan import lifespan_vector_store
...
async def test_lifespan_vector_store_initializes_and_cleans_up():
    ...
    with (
        patch(
            "app_vector_store.lifespan.setup_vector_store_provider", new_callable=AsyncMock
        ) as mock_setup_vector_store_provider,
        patch("app_vector_store.lifespan.close_vector_store", new_callable=AsyncMock) as mock_close_vector_store,
        patch("app_vector_store.lifespan.vector_store_cache") as mock_vector_store_cache,
    ):
