...
- `scroll(query_filter=...)` yields all matching payload records without vectors. `overwrite_payload(payload, selector)` preserves vectors. `delete(selector)` and payload updates accept native `PointIdsList` or `FilterSelector`.
