...
from app_file_storage.config import (
    ...
    NoneFileStorageSettings,
    S3FileStorageSettings,
)
...
async def test_create_client_local_provider(mock_local_settings):
    with patch("app_file_storage.factory.LocalStorageProvider.from_config", new_callable=AsyncMock) as mock_from_config:
...
async def test_create_client_s3_provider(mock_s3_settings):
    with patch("app_file_storage.factory.S3StorageProvider.from_config", new_callable=AsyncMock) as mock_from_config:
        mock_from_config.return_value = MagicMock(spec=FileStorageClient)
