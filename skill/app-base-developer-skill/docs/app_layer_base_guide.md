...
`app-prebuilt-outbox` ships one more: **`OutboxHook(outbox_repo, event_types)`**, which writes an outbox row in the same transaction as the change. Implement `payload(op, obj, identity)`. See its [README](../../../packages/prebuilt/app-prebuilt-outbox/README.md).
