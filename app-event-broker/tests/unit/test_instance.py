...
from app_event_broker.instance import (
    close_event_broker,
    get_event_broker,
    set_event_broker,
    setup_event_broker,
...
def reset_global_event_broker():
    ...
    import app_event_broker.instance as event_broker_instance
...
def test_get_event_broker_not_initialized():
    with (
        patch("app_event_broker.instance.get_event_broker_settings") as mock_get_settings,
        patch("app_event_broker.instance.setup_event_broker") as mock_setup,
    ):
...
def test_setup_event_broker_initializes(mock_broker):
    ...
    with patch(
        "app_event_broker.instance.EventBrokerFactory.create",
...
def test_setup_event_broker_already_initialized(mock_broker):
    ...
    with patch("app_event_broker.instance.EventBrokerFactory.create") as mock_create:
        setup_event_broker(settings=mock_settings)
