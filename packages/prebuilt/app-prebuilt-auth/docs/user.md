# Users, sessions and account approval
...
FastAPI and OAuth2 form parsing are included in the default installation. Use `install_auth(app)` to register all authentication routes together.
...
- `v1_users_router` (from `app_prebuilt_auth.user.api`) — the composed router
- `get_current_user`, `get_current_superuser`, `on_superuser`, `get_login_caller`, `get_login_lockout_listener` (from `app_prebuilt_auth.user.deps`)
...
Override `get_user_transaction` from `app_prebuilt_auth.user.database` with a dependency
...
The `app_prebuilt_auth.user.identities.ExternalIdentity` model stores provider-neutral issuer/subject links. All identity and Google flow tables are registered by default; apply them with the host migrations even when the provider is disabled.
