...
FastAPI is included. The host provides migrations and the interpretation of its machine scopes.
...
Importing `app_prebuilt_auth` registers `Machine`, `MachineKey` and all other auth models in the shared migration metadata. Their tables are `api_key_machines` and `api_keys`. Install the
...
`get_api_key_session_maker` (from `app_prebuilt_auth.api_key.database`) with an async
...
Use `get_machine_principal` from `app_prebuilt_auth.api_key.deps` on protected routes.
